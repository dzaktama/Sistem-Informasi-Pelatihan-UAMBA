...
try {
    $stmt = $pdo->prepare("SELECT m.id_materi, m.judul_materi, m.deskripsi_sesi, m.jadwal_sesi, (SELECT COUNT(*) FROM progres_materi pm WHERE pm.id_materi = m.id_materi AND pm.id_pengguna = ?) as sudah_selesai FROM materi m WHERE m.id_pelatihan = ? ORDER BY m.jadwal_sesi ASC");
                ...
                <a href="lihat_materi.php?id=<?php echo $materi['id_materi']; ?>" 
                   ...
                   style="animation-delay: <?php echo $index * 0.05; ?>s">
                    <div class="p-6 flex-1">
                        <span class="text-xs font-bold uppercase tracking-wider <?php echo $materi['sudah_selesai'] ? 'text-green-600' : 'text-amba-hijau-tua'; ?>">Sesi <?php echo $index + 1; ?></span>
                        <h3 class="text-xl font-bold text-gray-900 mt-2 mb-2"><?php echo htmlspecialchars($materi['judul_materi']); ?></h3>
                        <p class="text-gray-600 text-sm h-10"><?php echo htmlspecialchars($materi['deskripsi_sesi']); ?></p>
                    ...
                    <div class="bg-gray-50 p-4 border-t border-gray-100 flex justify-between items-center">
                        <?php if ($materi['sudah_selesai']): ?>
                            ...
                            <span class="py-2 px-4 rounded-md text-sm font-medium bg-white text-gray-700 border border-gray-300 hover:bg-gray-100">Lihat Lagi</span>
                        <?php else: ?>
                            <span class="text-sm font-medium text-gray-500">Belum Mulai</span>
                            <span class="py-2 px-4 rounded-md text-sm font-medium text-white bg-amba-hijau-tua hover:bg-amba-hijau-muda hover:text-amba-hijau-tua transition duration-200">Mulai Belajar</span>
                        <?php endif; ?>
                    </div>
                </a>
            <?php endforeach; ?>
